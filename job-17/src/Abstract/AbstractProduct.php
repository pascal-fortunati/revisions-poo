...
// Classe abstraite AbstractProduct implémente EntityInterface
abstract class AbstractProduct implements EntityInterface
{
    ...
    // Méthode pour sauvegarder le produit
    public function save(): self|false
    {
        // Le produit n'a pas d'ID, on le crée
        if ($this->id === 0) {
            return $this->create();
        }

        // Le produit a un ID, on vérifie s'il existe en base
        try {
            $pdo = new PDO('mysql:host=localhost;dbname=draft-shop;charset=utf8mb4', 'root', '');
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            ...
            // S'il existe, on met à jour, sinon on crée
            if ($exists) {
                return $this->update();
            } else {
                return $this->create();
