...
// Charger les produits depuis la base de données dans la collection interne
$category->loadProducts();
$productsCollection = $category->getProducts();

// Création d'une catégorie et ajout manuel de produits via addProduct()
$categoryTest = new Category(1, "Électronique", "Catégorie électronique");
$testProduct1 = new Electronic(10, "Test Laptop", [], 89900, "Un laptop de test", 5, 1, "Dell", 15000);
$testProduct2 = new Electronic(11, "Test Tablette", [], 45900, "Une tablette de test", 8, 1, "Apple", 10000);

// Ajout de produits à la catégorie
$categoryTest->addProduct($testProduct1);
$categoryTest->addProduct($testProduct2);
$categoryTestCount = $categoryTest->getProducts()->count();
...
<body>
    ...
    <div class="card" style="background-color: #d4edda; border-left: 4px solid #28a745;">
        <h2>Category avec propriété products (EntityCollection)</h2>
        <div class="info-line">
            <span class="label">Catégorie :</span> <?= htmlspecialchars($category->getName()) ?>
        </div>
        <div class="info-line">
            <span class="label">Produits chargés via loadProducts() :</span> <?= $productsCollection->count() ?>
        </div>
    ...
    <div class="card" style="background-color: #e3f2fd; border-left: 4px solid #2196F3;">
        <h2>Ajout de produits à une catégorie avec addProduct()</h2>
        <div class="info-line">
            <span class="label">Catégorie :</span> <?= htmlspecialchars($categoryTest->getName()) ?>
        </div>
        <div class="info-line">
            <span class="label">Produits ajoutés manuellement :</span> <?= $categoryTestCount ?>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php foreach ($categoryTest->getProducts()->getAll() as $prod): ?>
                    <?php if ($prod instanceof Electronic): ?>
                        <tr>
                            <td><?= $prod->getId() ?></td>
